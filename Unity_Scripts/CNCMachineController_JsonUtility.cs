...
/// <summary>
/// 五轴数控机床Unity控制器 (使用Unity内置JsonUtility版本)
/// 通过WebSocket接收FMU仿真数据并控制3D模型运动
/// </summary>
public class CNCMachineController_JsonUtility : MonoBehaviour
{
    [Header("机床组件引用")]
    ...
    // 机床状态
    private MachineStateJson machineState = new MachineStateJson();
    ...
    public void ReceiveTransformData(string jsonData)
    {
        try
        {
            var data = JsonUtility.FromJson<UnityTransformDataJson>(jsonData);
    ...
    /// 更新目标变换
    /// </summary>
    private void UpdateTargetTransforms(TransformsJson transforms)
    {
        if (transforms.table_x != null)
        {
    ...
    {
        var command = new AxisCommandJson
        {
            type = "move_axis",
            axis = axis,
            position = position
    ...
    {
        var command = new SpeedCommandJson
        {
            type = "set_speed",
            speed = speed
        };
    ...
    private IEnumerator SimulateDataInEditor()
    {
        while (true)
        {
            ...
            // 模拟仿真数据
            float t = Time.time;
            var simulatedData = new UnityTransformDataJson
            {
                timestamp = t,
                transforms = new TransformsJson
                {
                    table_x = new TransformInfoJson 
                    { 
                        position = new Vector3InfoJson { x = 10 * Mathf.Sin(0.1f * t), y = 0, z = 0 }
                    },
                    table_y = new TransformInfoJson 
                    { 
                        position = new Vector3InfoJson { x = 0, y = 0, z = 10 * Mathf.Cos(0.1f * t) }
                    },
                    spindle_z = new TransformInfoJson 
                    { 
                        position = new Vector3InfoJson { x = 0, y = 5 * Mathf.Sin(0.05f * t), z = 0 }
                    },
                    head_a = new TransformInfoJson 
                    { 
                        rotation = new Vector3InfoJson { x = 30 * Mathf.Sin(0.02f * t), y = 0, z = 0 }
                    },
                    table_c = new TransformInfoJson 
                    { 
                        rotation = new Vector3InfoJson { x = 0, y = 45 * Mathf.Cos(0.03f * t), z = 0 }
                    }
                },
                machine_state = new MachineStateJson { is_running = true }
            };
...
// JsonUtility兼容的数据结构定义
[System.Serializable]
public class UnityTransformDataJson
{
    public float timestamp;
    public TransformsJson transforms;
    public MachineStateJson machine_state;
}

[System.Serializable]
public class TransformsJson
{
    public TransformInfoJson table_x;
    public TransformInfoJson table_y;
    public TransformInfoJson spindle_z;
    public TransformInfoJson head_a;
    public TransformInfoJson table_c;
}

[System.Serializable]
public class TransformInfoJson
{
    public Vector3InfoJson position = new Vector3InfoJson();
    public Vector3InfoJson rotation = new Vector3InfoJson();
    public Vector3InfoJson scale = new Vector3InfoJson { x = 1, y = 1, z = 1 };
}

[System.Serializable]
public class Vector3InfoJson
{
    public float x;
    public float y;
    public float z;
...
public class MachineStateJson
{
    ...
    public bool alarm;
    public bool ready;
    public ToolInfoJson tool_info = new ToolInfoJson();
}

[System.Serializable]
public class ToolInfoJson
{
